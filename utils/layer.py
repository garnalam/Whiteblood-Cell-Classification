...
def initializer_conv2d(in_channels, out_channels, mapsize, stddev_factor=1.0, mode="Glorot"):
    ...
    return tf.random.truncated_normal([mapsize, mapsize, in_channels, out_channels],
...
def conv2d(x, in_channels, out_channels, kernel_size=3, stride=1, padding="SAME", trainable=True, verbose=False):
    ...
    if verbose:
        print("|--------------------------------------- Conv2D ----------------------------------------|")
        print("| feature size: {0: <28}  filter size: {1: <29}|".format(
            str(out.shape), 
            str(weight.shape)
        ))
...
def upsampling2d(x, size=(2, 2), verbose=False):
    ...
    if verbose:
        print("|------------------------------------ Upsampling2D -------------------------------------|")
        print("| feature size: {0: <72}|".format(str(out.shape)))
...
def prelu(x, trainable=True, verbose=False):
    ...
    '''
    alpha = tf.Variable(
    initial_value=tf.zeros(x.shape[-1]),
    name="alpha",
    dtype=tf.float32,
    trainable=trainable
    )
...
def batchnorm(x, is_training, decay=0.99, epsilon=0.001, trainable=True, verbose=False):
    ...
    beta = tf.Variable(
    initial_value=tf.random.truncated_normal([dim], stddev=0.0),
    name='beta',
    dtype=tf.float32,
    trainable=trainable
    )
    scale = tf.Variable(
    initial_value=tf.random.truncated_normal([dim], stddev=0.1),
    name='scale',
    dtype=tf.float32,
    trainable=trainable
    )
    pop_mean = tf.Variable(
    initial_value=tf.zeros([dim]),
    name='pop_mean',
    dtype=tf.float32,
    trainable=False
    )
    pop_var = tf.Variable(
    initial_value=tf.ones([dim]),
    name='pop_var',
    dtype=tf.float32,
    trainable=False
    )
...
def fullyconnected(x, num_out, trainable=True, verbose=False):
    ...
    W = tf.Variable(
    initial_value=tf.random.truncated_normal([num_in, num_out], stddev=0.1),
    name="weight",
    dtype=tf.float32,
    trainable=trainable
    )
    b = tf.Variable(
    initial_value=tf.zeros([num_out]),
    name="bias",
    dtype=tf.float32,
    trainable=trainable
    )
